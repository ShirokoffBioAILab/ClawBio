...
def make_receipt(request: dict[str, Any], *, profile: str | None, region: str) -> SubmissionReceipt:
    return {"schema_version": 2, "state": "SUBMITTING", "request": request,
            "request_sha256": request_digest(request), "profile": profile,
            "region": region, "run_id": None}
...
def load_receipt(path: Path, *, profile: str | None, region: str) -> dict[str, Any]:
    ...
    if receipt.get("schema_version") == 2 and receipt.get("request_sha256") != request_digest(request):
